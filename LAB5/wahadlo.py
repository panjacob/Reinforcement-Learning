...
def wahadlo_uczenie(episode_count=50_000, alpha=0.001, gamma=0.998, epsilon=0.5):
    max_steps = 1000
    ...
    for episode in range(episode_count):
        ...
        for i in range(max_steps):
            ...
            for s0, s1, s2, s3, a in near_states:
                gradient = one_hot_encoding_state([s0, s1, s2, s3], gradient)

            if abs(state_next[0]) >= np.pi / 2 or abs(state_next[2]) > BIN_MAX[2]:
                break
            alpha = 1 / ((len(near_states) + 1) * 40)
            mesh.W[:, :, a] += alpha * (r + gamma * r_next_hat - r_hat) * gradient
            mesh.W[:, :, a] = mesh.W[:, :, a] / np.linalg.norm(mesh.W[:, :, a])
            state = state_next
